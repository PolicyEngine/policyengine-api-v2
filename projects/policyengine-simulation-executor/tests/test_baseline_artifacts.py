...
class SPMWrapperSimulation(Simulation):
    ...
    is all the guard depends on. It does not prove the deployed wrapper
    emits receipts of this shape — only the native tests do.
    """
...
def _make_spm_sim(model_version, *, spm=None, sim_id="bl1-spm", year=2026):
...
class TestEnsureValidatesSPMReceipts:
    ...
    def test_stale_cached_selection_cannot_replace_the_request(self, fresh_cache):
        """A cache hit restores the cached entry's selection onto this
        request. The guard validates against the pre-load selection, so the
        stale entry can only cost a recompute — never silently answer the
        request under the wrong SPM settings."""
        other = {**SPM_SELECTION, "scenario": "zero_real"}
        stale = _make_spm_sim(SPMModelVersion(), spm=other)
        stale.output_dataset = _output(_complete_frames())
        stale.spm_config = other
        stale.spm_receipt = _spm_receipt(other)
        fresh_cache.add("bl1-spm", stale)
    ...
    def test_recompute_replaces_the_cache_with_this_request_s_selection(
        ...
        sim = _make_spm_sim(model)
        sim.ensure()
        assert fresh_cache.get("bl1-spm").spm_config == SPM_SELECTION
